-- Fix fee_payments schema cache issue
-- Run this in your Supabase SQL Editor to resolve the 'component_type_id' column error

-- 1. First, let's check what columns currently exist in fee_payments
DO $$
DECLARE
    col_record RECORD;
BEGIN
...
-- 2. Drop and recreate the fee_payments table with the correct schema
DROP TABLE IF EXISTS fee_receipts CASCADE;
DROP TABLE IF EXISTS fee_payments CASCADE;

-- 3. Create fee_payments table with the correct schema
...
-- 8. Create fee collection summary view
...
DECLARE
    col_record RECORD;
BEGIN
    RAISE NOTICE 'Updated fee_payments columns:';
    FOR col_record IN 
        SELECT column_name, data_type, is_nullable
        FROM information_schema.columns 
        WHERE table_name = 'fee_payments' 
...
-- 12. Clear any schema cache issues by refreshing the schema
