-- Fee Collections Migration
-- Adds tables for tracking fee payments, receipts, and collection progress

-- Fee payments table (tracks individual payments)
CREATE TABLE IF NOT EXISTS fee_payments (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  student_id uuid NOT NULL REFERENCES student(id) ON DELETE CASCADE,
  plan_id uuid NOT NULL REFERENCES fee_student_plans(id) ON DELETE CASCADE,
...
LEFT JOIN fee_payments fp ON fspi.plan_id = fp.plan_id AND fspi.component_type_id = fp.component_type_id
...
BEGIN
  ...
  SELECT school_code INTO school_code FROM users WHERE id = auth.uid();
