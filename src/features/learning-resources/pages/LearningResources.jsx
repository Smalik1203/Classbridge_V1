...
import ClassDetailView from '@/features/students/components/ClassDetailView';
import SubjectFilter from '@/features/students/components/SubjectFilter';
...
const LearningResources = () => {
  ...
  // Memoize filtered resources to avoid recalculating on every render
  const filteredResources = useMemo(() => {
    if (!Array.isArray(resources)) return [];
    if (selectedType === 'all') return resources;
    return resources.filter(resource => resource?.resource_type === selectedType);
  }, [resources, selectedType]);

  // Memoize sorted resources based on selected sort option
  const sortedResources = useMemo(() => {
    if (!Array.isArray(filteredResources)) return [];
  ...
  // Keep these functions for backwards compatibility if called elsewhere
